			...
			struct stats_record *stats_prev)
{
	...
	printf("--------------------------------------------------------------\n");
	for (act = 0; act < XDP_ACTION_MAX; act++) {
		char *fmt = "%-12s %'11lld pkts (%'10.0f pps)"
			" %'11lld Kbytes (%'6.0f Mbits/s)"
			" period:%f\n";
		const char *action = action2str(act);
		rec  = &stats_rec->stats[act];
		prev = &stats_prev->stats[act];
		...
		bytes = (rec->total.rx_bytes - prev->total.rx_bytes)/1024;
		bps = bytes / 0.008;

		printf(fmt, action, rec->total.rx_packets, pps, bytes, bps,
			period);
	}
}
...
void map_get_value_percpu_array(int fd, __u32 key, struct datarec *value)
{
	...
	if ((bpf_map_lookup_elem(fd, &key, values)) != 0) {
		fprintf(stderr,
			"ERR: bpf_map_lockup_elem failed key:0x%X\n", key);
		return;
	}

	/* sum values for each cpu */
	for (i = 0; i < nr_cpus; i++) {
